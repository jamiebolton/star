...
import club.crazypenguin.service.sys.SyorganizationServiceI;
import club.crazypenguin.utils.BeanUtils;
...
public class SyorganizationServiceImpl extends BaseServiceImpl<Syorganization> implements SyorganizationServiceI {
    ...
    public void updateOrganization(Syorganization syorganization) {
        if (!StringUtils.isBlank(syorganization.getId())) {
            ...
            // 说明要修改的节点选中了上级节点
            if (syorganization.getSyorganization() != null) {
                // 上级节点
                Syorganization pt = getById(syorganization.getSyorganization().getId());

                // 说明要将当前节点修改到当前节点的子或者孙子下
                isParentToChild(t, pt, oldParent);
