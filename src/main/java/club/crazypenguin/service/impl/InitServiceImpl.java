...
import club.crazypenguin.model.*;
import club.crazypenguin.service.InitServiceI;
import club.crazypenguin.utils.MD5Util;
...
public class InitServiceImpl implements InitServiceI {
	...
	private void initOrganization(Document document) {
		...
		for (Object obj : childNodes) {
			...
			if (!StringUtils.isBlank(node.valueOf("@pid"))) {
				organization.setSyorganization((Syorganization) baseDao.getById(Syorganization.class,
						node.valueOf("@pid")));
			} else {
	...
	private void initUser(Document document) {
		...
		for (Node node : childNodes) {
			...
			List<Syuser> ul = baseDao.find("from Syuser u where u.loginname = '" + user.getLoginname()
					+ "' and u.id != '" + user.getId() + "'");
			for (Syuser u : ul) {
				baseDao.delete(u);
			}
	...
	private void initUserOrganization(Document document) {
		...
		for (Node node : childNodes) {
			...
			if (user != null) {
				...
				for (Node n : cNodes) {
					Syorganization organization = (Syorganization) baseDao.getById(Syorganization.class,
							n.valueOf("@id"));
					if (organization != null) {
